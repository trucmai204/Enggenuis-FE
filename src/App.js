...
function App() {
  ...
  const ProtectedRoute = ({ element }) => {
    useEffect(() => {
      if (!isLoggedIn) {
        setNotification("Bạn cần đăng nhập để sử dụng EngGenius");
      }
    }, [isLoggedIn]); // Chỉ chạy khi isLoggedIn thay đổi
  ...
  return (
    <>
      ...
      <Routes>
        ...
        <Route
          path="/dictionary"
          element={<ProtectedRoute element={<Frame2 />} />}
        />
        <Route path="/register" element={<RegisterPage />} />
        <Route path="/test" element={<ProtectedRoute element={<Frame4 />} />} />
        <Route
          path="/advisory"
          element={<ProtectedRoute element={<ChatbotInterface />} />}
        />
        <Route
          path="/history"
          element={<ProtectedRoute element={<Frame7 />} />}
        />
        <Route
          path="/search"
          element={<ProtectedRoute element={<Frame9 />} />}
        />
        <Route
          path="/account"
          element={<ProtectedRoute element={<Frame10 />} />}
        />
        <Route
          path="/DoTest"
          element={<ProtectedRoute element={<DoTest />} />}
        />
        <Route
          path="/AccountInfoPage"
          element={<ProtectedRoute element={<AccountInfoPage />} />}
        />
        <Route
          path="/PaymentPage"
